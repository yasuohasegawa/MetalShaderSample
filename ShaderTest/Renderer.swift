...
class Renderer: NSObject, MTKViewDelegate {
    ...
    init?(metalKitView: MTKView) {
        ...
        self.resolutionBuffer = self.device.makeBuffer(bytes: resolutionData, length: resolutionSize, options: [])!
    ...
    private func updateGameState() {
        ...
        let modelMatrix = matrix4x4_rotation(radians: Float(90.0*Double.pi/180.0), axis: rotationAxis)
        let viewMatrix = matrix4x4_translation(0.0, 0.0, -3.5)
        let scaleMatrix = matrix4x4_scale(1.0,1.0,2.1);
        let vm = simd_mul(viewMatrix, modelMatrix);
        
        uniforms[0].modelViewMatrix = simd_mul(vm, scaleMatrix)
        //rotation += 0.01
    ...
    func mtkView(_ view: MTKView, drawableSizeWillChange size: CGSize) {
        ...
        projectionMatrix = matrix_perspective_right_hand(fovyRadians: radians_from_degrees(60), aspectRatio:aspect, nearZ: 0.1, farZ: 100.0)
    }
}
